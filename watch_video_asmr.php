...
if(isset($_POST['like_asvideo'])){

   if($user_id != ''){

      $asvideo_id = $_POST['asvideo_id'];
      $asvideo_id = filter_var($asvideo_id, FILTER_SANITIZE_STRING);

      $select_asvideo = $conn->prepare("SELECT * FROM `asvideo` WHERE id = ? LIMIT 1");
      $select_asvideo->execute([$asvideo_id]);
      $fetch_asvideo = $select_asvideo->fetch(PDO::FETCH_ASSOC);

      $tutor_id = $fetch_asvideo['tutor_id'];

      /* $select_likes = $conn->prepare("SELECT * FROM `likes` WHERE user_id = ? AND asvideo_id = ?");
      $select_likes->execute([$user_id, $asvideo_id]);

      if($select_likes->rowCount() > 0){
         $remove_likes = $conn->prepare("DELETE FROM `likes` WHERE user_id = ? AND asvideo_id = ?");
         $remove_likes->execute([$user_id, $asvideo_id]);
         $message[] = 'removed from likes!';
      }else{
         $insert_likes = $conn->prepare("INSERT INTO `likes`(user_id, tutor_id, asvideo_id) VALUES(?,?,?)");
         $insert_likes->execute([$user_id, $tutor_id, $asvideo_id]);
...
/* 
      if($select_asvideo->rowCount() > 0){
         ...
         $select_comment = $conn->prepare("SELECT * FROM `comments` WHERE asvideo_id = ? AND user_id = ? AND tutor_id = ? AND comment = ?");
         $select_comment->execute([$asvideo_id, $user_id, $tutor_id, $comment_box]);

         if($select_comment->rowCount() > 0){
            $message[] = 'comment already added!';
         }else{
            $insert_comment = $conn->prepare("INSERT INTO `comments`(id, asvideo_id, user_id, tutor_id, comment) VALUES(?,?,?,?,?)");
            $insert_comment->execute([$id, $asvideo_id, $user_id, $tutor_id, $comment_box]);
...
<section class="watch-video">
   ...
   <?php
      $select_asvideo = $conn->prepare("SELECT * FROM `asvideo` WHERE id = ? AND status = ?");
      $select_asvideo->execute([$get_id, 'active']);
      if($select_asvideo->rowCount() > 0){
         while($fetch_asvideo = $select_asvideo->fetch(PDO::FETCH_ASSOC)){
            $asvideo_id = $fetch_asvideo['id'];

            /* $select_likes = $conn->prepare("SELECT * FROM `likes` WHERE asvideo_id = ?");
            $select_likes->execute([$asvideo_id]);
            $total_likes = $select_likes->rowCount();  

            $verify_likes = $conn->prepare("SELECT * FROM `likes` WHERE user_id = ? AND asvideo_id = ?");
            $verify_likes->execute([$user_id, $asvideo_id]); */

            $select_tutor = $conn->prepare("SELECT * FROM `tutors` WHERE id = ? LIMIT 1");
            $select_tutor->execute([$fetch_asvideo['tutor_id']]);
            $fetch_tutor = $select_tutor->fetch(PDO::FETCH_ASSOC);
   ?>
   <div class="video-details">
      <video src="uploaded_files/<?= $fetch_asvideo['video']; ?>" class="video" poster="uploaded_files/<?= $fetch_asvideo['thumb']; ?>" controls autoplay></video>
      <h3 class="title"><?= $fetch_asvideo['title']; ?></h3>
      <div class="info">
         <p><i class="fas fa-calendar"></i><span><?= $fetch_asvideo['date']; ?></span></p>
      ...
      <form action="" method="post" class="flex">
         <input type="hidden" name="asvideo_id" value="<?= $asvideo_id; ?>">
         <a href="playlist_asmr.php?get_id=<?= $fetch_asvideo['asmr_id']; ?>" class="inline-btn">view video</a>
         <!-- <?php
      ...
      <div class="description"><p><?= $fetch_asvideo['description']; ?></p></div>
   </div>
   <?php
         }
      }else{
