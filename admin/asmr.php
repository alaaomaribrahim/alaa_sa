...
if(isset($_POST['delete_video'])){
   ...
   if($verify_video->rowCount() > 0){
     ...
     /*  $delete_comments = $conn->prepare("DELETE FROM `comments` WHERE asvideo_id = ?");
      ...
      $delete_asmrs = $conn->prepare("DELETE FROM `asvideo` WHERE id = ?");
      $delete_asmrs->execute([$delete_id]); 
...
<section class="contents">
   ...
   <h1 class="heading">your asmr</h1>

   <div class="box-container">

   <div class="box" style="text-align: center;">
      <h3 class="title" style="margin-bottom: .5rem;">create new asmr video</h3>
      <a href="add_asmr.php" class="btn">add asmr video</a>
   </div>

   <?php
      $select_videos = $conn->prepare("SELECT * FROM `asvideo` WHERE tutor_id = ? ORDER BY date DESC");
   ...
   ?>
      <div class="box">
         ...
         <a href="view_asmr.php?get_id=<?= $video_id; ?>" class="btn">view asmr video</a>
      </div>
   <?php
         }
      }else{
         echo '<p class="empty">no asmrs added yet!</p>';
      }
   ?>
