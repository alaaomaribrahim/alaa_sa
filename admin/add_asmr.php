...
if(isset($_POST['submit'])){
   ...
   if($thumb_size > 2000000){
      ...
      $add_asmr = $conn->prepare("INSERT INTO `asvideo`(id, tutor_id, asmr_id, title, description, video, thumb, status) VALUES(?,?,?,?,?,?,?,?)");
      $add_asmr->execute([$id, $tutor_id, $asmr, $title, $description, $rename_video, $rename_thumb, $status]); 
...
<section class="video-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <p>video ASMR <span>*</span></p>
      <select name="asmr" class="box" required>
         <option value="" disabled selected>--select ASMR</option>
         <?php
         $select_asmrs = $conn->prepare("SELECT * FROM `asmr` WHERE tutor_id = ?");
         $select_asmrs->execute([$tutor_id]);
         if($select_asmrs->rowCount() > 0){
            while($fetch_asmr = $select_asmrs->fetch(PDO::FETCH_ASSOC)){
         ?>
